...
| --- | --- | --- | --- | --- |
| Monas standalone host, product mount, and authentication | `../monas` `c91c544a890769f670a4b398dfcfe8a8cf8185b1` (workspace `0.8.2`) | `README.md`; `crates/monas-core/src/lib.rs` (`ProductMount`, `ProductBootstrapResponse`, `HostInfoResponse`); `crates/monas-core/src/auth.rs`; `crates/monas-server/src/{lib.rs,main.rs}` | `monas_standalone`; `/opt/<productName>`; `/api/auth/{register,login,logout,session}`; `monas_session`; validated product-aware sign-on presentation; digest-guarded legacy identity migration; process-start session revocation; hop-by-hop-safe Pinakotheke forwarding; x-img `contracts/monas/x-img-product-bootstrap.v1.json`; x-img `fixtures/host-context/v1/monas-valid.json`; tests `host_info_declares_standalone_storage_boundary`, `login_page_uses_validated_pinakotheke_destination_branding`, `product_shell_requires_cookie_session`, `startup_migrates_legacy_identity_records_once`, and `startup_invalidates_sessions_issued_by_an_earlier_process` | x-img consumes authenticated host context and host-relative mounts; it does not implement login, cookies, or session issuance. Monas upgrades legacy local identities through Prosopikon's reviewed migration contract, revokes pre-start browser sessions, and forwards authenticated GET/HEAD traffic without unsafe request framing. **Status: legacy upgrade, registration, host context, sign-on branding, restart invalidation, and authenticated forwarding contracts complete.** |
